#!/usr/bin/env bash
# ---------------------------------------------------------------------------
# Populate ./deps/ so that targets/target-310p/Dockerfile.aarch64 can be built
# with `--network=none`. This is the ONLY step that touches the network.
#
#   ./targets/target-310p/provision.sh [deps-dir]      (default: <repo>/deps)
#
# Produces:
#
#   deps/
...
#
# Seeding: point the matching variable at a local copy and the download or
# build for it is skipped:
#
...
COMMON_DIR="${COMMON_DIR:-$REPO_DIR/common}"
DEPS="${1:-${DEPS_DIR:-$REPO_DIR/deps}}"
TARGET_DIR="${TARGET_DIR:-$REPO_DIR/targets/target-310p}"
...
# Staged once, up front: the in-container steps bind-mount $PROV as /prov and
# common/scripts as /common, and read these under their canonical names.
[ -d "$COMMON_DIR/scripts" ] \
    || die "missing $COMMON_DIR/scripts - run from a complete checkout"
cp -f "$TARGET_DIR/packages.aarch64.txt"              "$PROV/packages.txt"
cp -f "$TARGET_DIR/requirements.aarch64.txt"          "$PROV/requirements.txt"
cp -f "$TARGET_DIR/requirements-optional.aarch64.txt" "$PROV/requirements-optional.txt"
cp -f "$TARGET_DIR/constraints.aarch64.txt"           "$PROV/constraints.txt"
...
# which cannot happen inside a --network=none image build. Every one
...
# is what keeps the op build offline:
#
...
if [ -f "$DEBS/Packages.gz" ]; then
    ...
    drun -e EMULATED=1 \
         ...
         -v "$PROV:/prov:ro" -v "$DEBS:/out" \
         "$ARM_BASE" bash /common/fetch_debs.sh
fi
...
if ls "$WHEELS"/vllm-*.whl >/dev/null 2>&1; then
    ...
    # No pre-seeded torch here, unlike the 950PR target, so the +cpu wheels the
    # constraint pins must come from download.pytorch.org during the resolve.
    # BUILD_DEPS omits setuptools-rust: vLLM v0.13.0's setup.py does not import
    # it, and under QEMU every avoidable install costs minutes.
    drun -e EMULATED=1 \
         -e PLAT_NAME=manylinux2014_aarch64 \
         -e CONSTRAINTS=/prov/constraints.txt \
         -e EXTRA_INDEXES=https://download.pytorch.org/whl/cpu \
         -e SETUPTOOLS_SPEC='setuptools>=64,<81.0.0' \
         -e BUILD_DEPS='torch cmake ninja packaging jinja2 regex' \
         -v "$COMMON_DIR/scripts:/common:ro" \
         -v "$PROV:/prov:ro" -v "$SRC:/src" -v "$WHEELS:/out" \
         "$ARM_BASE" bash /common/build_vllm_wheel.sh
fi
...
# ---------------------------------------------------------------------------
# Pass 3 retries requirements-optional.txt one entry at a time. None of those is
# needed for `import vllm_ascend` or for `vllm serve` on a 310P3, so a mirror
# miss is reported and does not fail the stage.
drun -e EMULATED=1 \
     -e ENFORCE_CPU_TORCH=1 \
     -e CONSTRAINTS=/prov/constraints.txt \
     -e OWN_REQ=/prov/requirements.txt \
     -e PLUGIN_REQ=/src/vllm-ascend/requirements.txt \
     -e MIRROR_ONLY=/prov/requirements-optional.txt \
     -e EXTRA_INDEXES=https://download.pytorch.org/whl/cpu \
     -v "$COMMON_DIR/scripts:/common:ro" \
     -v "$PROV:/prov:ro" -v "$SRC:/src:ro" -v "$WHEELS:/wheels" \
     "$ARM_BASE" bash /common/fetch_wheels.sh
...
echo "ready: ./targets/target-310p/build.sh"
