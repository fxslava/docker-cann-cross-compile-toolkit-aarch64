...
#
#   ./targets/target-310p/build.sh                       build and load into the local daemon
#   ./targets/target-310p/build.sh --save out.tar.gz     build, then export for air-gapped copy
#   ./targets/target-310p/build.sh --no-cache            anything unrecognised goes to buildx
#
# Environment:
...
#   TARGET_DIR image assets     (default $CONTEXT/targets/target-310p)
#
...
TARGET_DIR="${TARGET_DIR:-$CONTEXT/targets/target-310p}"
...
docker buildx inspect --bootstrap 2>/dev/null | grep -qE "^Platforms:.*\blinux/arm64\b" \
    || die "buildx does not offer linux/arm64"
...
# still reach a registry to fetch it; on an air-gapped builder, cache it first.
#
# FAILURE MODE - `docker pull --platform linux/arm64 ubuntu:22.04` REPLACES the
# local ubuntu:22.04 tag with the arm64 image and breaks the x86_64 builder
# image. Pre-pull arm64v8/ubuntu:22.04 and pass it through instead:
...
if [ "${#missing[@]}" -gt 0 ]; then
    ...
    printf '  - %s\n' "${missing[@]}" >&2
    die "run ./targets/target-310p/provision.sh ${DEPS_DIR} first"
fi
