...
# works; sourcing the set_env.sh scripts on top picks up anything
# version-specific. nnal/atb is what upstream's Dockerfile.a5 sources before
# touching vllm-ascend, so it is not optional on this target.
set +u
...
# NO DTYPE IS FORCED HERE, and that is the deliberate difference from the 310P
# entrypoint. The 310P pins --dtype float16 because its AI Core has no
# bfloat16 path at all, so an unqualified serve of a bf16 checkpoint fails.
# DaVinci v3 has bf16, so the checkpoint's own dtype is the right default and
# overriding it would silently degrade quality. Set VLLM_DTYPE to override.
